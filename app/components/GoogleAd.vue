<template>
    <div v-if="!disabled && runtimeConfig.public.adSenseEnabled !== false && adState !== 'blocked'" class="w-full max-w-[800px] mx-auto" :class="[adState === 'loaded' ? ['border-2 border-yellow-400 p-2 rounded-lg overflow-hidden', props.customClass] : 'relative h-0 overflow-hidden']">
        <ins
            ref="insEl"
...
onMounted(() => {
    // Skip ad initialization if disabled
    if (props.disabled || runtimeConfig.public.adSenseEnabled === false) return;
