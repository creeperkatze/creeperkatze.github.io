<template>
    <div class="flex flex-col items-center justify-center space-y-4">
        <h1 class="text-3xl font-bold">{{ $t('page.tools.tool.color-converter.title') }}</h1>
        ...
        <div class="w-full max-w-4xl p-6 glass-effect border-2 rounded-lg space-y-8">
            ...
            <div class="flex flex-col md:flex-row gap-6">
                <div class="w-full md:w-1/3 aspect-square rounded-2xl shadow-lg relative overflow-hidden group border-2 border-white/20">
                    <div class="absolute inset-0 transition-colors duration-200" :style="{ backgroundColor: hexString }"></div>
                    <input type="color" v-model="hexString" class="absolute inset-0 opacity-0 cursor-pointer w-full h-full" />
                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <span class="bg-black/50 text-white px-3 py-1 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div class="space-y-4">
                <h2 class="text-xl font-bold">{{ $t('page.tools.tool.color-converter.schemes') }}</h2>

                <div class="space-y-2">
                    <h3 class="text-sm text-gray-300">{{ $t('page.tools.tool.color-converter.scheme.complementary') }}</h3>
                    <div class="flex h-16 rounded-lg overflow-hidden border border-white/10 cursor-pointer">
                        ...
                        <div class="flex-1 transition-colors hover:opacity-90" 
                             ...
                             @click="setColor(complementaryColor)"
                ...
                <div class="space-y-2">
                    <h3 class="text-sm text-gray-300">{{ $t('page.tools.tool.color-converter.scheme.analogous') }}</h3>
                    <div class="flex h-16 rounded-lg overflow-hidden border border-white/10 cursor-pointer">
                        <div v-for="color in analogousColors" :key="color" 
                             class="flex-1 transition-colors hover:opacity-90"
                ...
                <div class="space-y-2">
                    <h3 class="text-sm text-gray-300">{{ $t('page.tools.tool.color-converter.scheme.triadic') }}</h3>
                    <div class="flex h-16 rounded-lg overflow-hidden border border-white/10 cursor-pointer">
            ...
            <div class="space-y-4">
                <h2 class="text-xl font-bold">{{ $t('page.tools.tool.color-converter.variations') }}</h2>
                <div class="flex flex-col gap-2">
                    <div class="flex h-12 rounded-lg overflow-hidden border border-white/10">
            ...
            <div class="space-y-4">
                <h2 class="text-xl font-bold">{{ $t('page.tools.tool.color-converter.accessibility') }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-4 rounded-lg flex items-center justify-between" :style="{ backgroundColor: hexString }">
                        <span class="text-white font-bold text-lg">{{ $t('page.tools.tool.color-converter.text_color') }} (White)</span>
                        <div class="bg-black/50 backdrop-blur-md px-3 py-1 rounded text-white text-sm">
                            {{ contrastWhite.toFixed(2) }}:1
                            <span v-if="contrastWhite >= 7" class="text-green-400 ml-1 font-bold">AAA</span>
                            <span v-else-if="contrastWhite >= 4.5" class="text-green-400 ml-1 font-bold">AA</span>
                            <span v-else class="text-red-400 ml-1 font-bold">Fail</span>
                        </div>
                    </div>
                    <div class="p-4 rounded-lg flex items-center justify-between" :style="{ backgroundColor: hexString }">
                        <span class="text-black font-bold text-lg">{{ $t('page.tools.tool.color-converter.text_color') }} (Black)</span>
                        <div class="bg-white/50 backdrop-blur-md px-3 py-1 rounded text-black text-sm">
                            {{ contrastBlack.toFixed(2) }}:1
                            <span v-if="contrastBlack >= 7" class="text-green-600 ml-1 font-bold">AAA</span>
                            <span v-else-if="contrastBlack >= 4.5" class="text-green-600 ml-1 font-bold">AA</span>
                            <span v-else class="text-red-600 ml-1 font-bold">Fail</span>
                        </div>
                    </div>
                </div>
            </div>
